class GroceryListsController < ApplicationController
  before_action :set_grocery_list, only: [:show, :edit, :update, :destroy, :complete, :archive, :share]
  ...
  def share
    email = params[:email]
    if email.present?
      UserMailer.grocery_list_share(current_user, @grocery_list, email).deliver_later
      redirect_to @grocery_list, notice: "Grocery list shared with #{email}."
    else
      redirect_to @grocery_list, alert: 'Please provide an email address.'
    end
  end
